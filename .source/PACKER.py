...
try:
    ...
    except Exception as error: errorContent = ('Module load'); errorCode = ('Classic'); displayError(error)
    ...
    except FileNotFoundError as error: errorCode = ('MissingFile'); displayError(error)
    except KeyError as error: errorCode = ('KeyError'); displayError(error)
    except Exception as error: errorCode = ('Unknown'); displayError(error)


    try:
        with open(servers_yml, 'r') as file: servers_config = yaml.safe_load(file) # load yaml/yml file (servers.yml)
    except FileNotFoundError as error:
        errorContent = ('servers.yml'); errorCode = ('MissingFile'); displayError(error)
