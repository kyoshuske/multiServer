    ...
    if arg != 'nopack':
        ...
        for server in servers_config['servers']:
            ...
            try:
                ...
                server['arguments'] = 'cd /D "' + server['path'] + '"\n"' + java + '" -Xmx' + str(server['max-heap-size']) + ' -jar "' + server['jar-file'] + '"' + str(prt) + visual['nogui'] + visual['noconsole'] + file['server-properties'] + file['bukkit'] + file['spigot'] + file['paper'] + plugins
